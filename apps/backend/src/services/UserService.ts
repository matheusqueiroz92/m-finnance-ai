...
export class UserService implements IUserService {
   ...
   * @param profile - The profile data to login with
   * @returns A promise that resolves to the user and token
   */
  async loginWithSocialProvider(profile: ISocialUser): Promise<IAuthResult> {
    return TransactionManager.executeInTransaction(async (session) => {
      ...
      if (!email && provider === "github") {
        ...
        console.warn(
          `Usuário ${provider} sem email disponível: ${profile.id}. Usando email temporário.`
        );
      }
      ...
      console.info(
        `Login social bem-sucedido: ${provider} para usuário ${user._id} (${user.email})`
      );
   ...
   * @returns A promise that resolves to the users and total
   */
  async getAllUsers(
    page: number = 1,
    limit: number = 10,
    ...
    return TransactionManager.executeInTransaction(async (session) => {
      ...
      if (updateData.email) {
        ...
        if (
          userWithEmail &&
          userWithEmail._id &&
          userWithEmail._id.toString() !== userId
        ) {
   ...
   * @param passwordData - The data to change the password with
  ...
  async changePassword(
    ...
    return TransactionManager.executeInTransaction(async (session) => {
      ...
      const isPasswordValid = await this.userRepository.checkPassword(
        userId,
        currentPassword
      );
   ...
   * @param token - The token to verify the email with
  ...
  private async sendVerificationEmail(user: IUser): Promise<void> {
    ...
    if (!userId || !user.emailVerificationToken) {
      console.error(
        "Erro ao enviar e-mail de verificação: dados de usuário incompletos"
      );
      return;
    }

    const verificationUrl = `${process.env.FRONTEND_URL}/verify-email-token?token=${user.emailVerificationToken}`;

    const emailSubject = "Verifique seu e-mail - M. Finnance AI";
    const emailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2>Olá, ${user.name}!</h2>
        <p>Bem-vindo ao M. Finnance AI. Por favor, confirme seu endereço de e-mail clicando no botão abaixo:</p>
        ...
        <p>Se você não se cadastrou no M. Finnance AI, ignore este e-mail.</p>
   ...
   * @param user - The user to remove the sensitive data from
