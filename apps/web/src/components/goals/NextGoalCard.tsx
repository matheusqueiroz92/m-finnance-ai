...
export function NextGoalCard({ goal, isLoading }: NextGoalCardProps) {
  ...
  const getGoalIcon = (name: string, icon?: string) => {
    ...
    if (nameLower.includes("viagem") || nameLower.includes("férias")) {
      return (
        <Plane
          className={`h-6 w-6 ${isDark ? "text-blue-400" : "text-blue-500"}`}
        />
      );
    } else if (nameLower.includes("carro") || nameLower.includes("veículo")) {
      return (
        <Car
          className={`h-6 w-6 ${isDark ? "text-indigo-400" : "text-indigo-500"}`}
        />
      );
    } else if (
      ...
      return (
        <Home
          className={`h-6 w-6 ${isDark ? "text-emerald-400" : "text-emerald-500"}`}
        />
      );
    } else if (nameLower.includes("estudo") || nameLower.includes("curso")) {
      return (
        <Briefcase
          className={`h-6 w-6 ${isDark ? "text-orange-400" : "text-orange-500"}`}
        />
      );
    }
      ...
      <CardContent className="p-6">
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <Progress
              value={goal.progress}
              className={`h-2 mt-2 ${isDark ? "bg-white/10" : "bg-gray-200"}`}
            />
          </div>
        </div>
      </CardContent>
