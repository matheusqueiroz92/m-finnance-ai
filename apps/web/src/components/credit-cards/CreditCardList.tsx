...
import { formatExpiryDate, getCardBrandColor, getCardBrandLogo, getCardBrandName, getDaysUntilDue } from "@/lib/functions";
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {[...Array(4)].map((_, i) => (
          <Card
            key={i}
            className="animate-pulse bg-gray-200 dark:bg-gray-700 h-56"
          ></Card>
        ))}
      </div>
    );
      ...
      {creditCards.map((card) => (
        <Card key={card._id} className="overflow-hidden shadow">
          <CardContent className="p-0">
            ...
            <div
              className={`bg-gradient-to-r ${getCardBrandColor(card.cardBrand, isDark)} p-6 text-white`}
            >
                ...
                <div className="h-8 w-12">
                  {getCardBrandLogo(card.cardBrand) ? (
                    ...
                    <span className="text-sm font-medium">
                      {getCardBrandName(card.cardBrand)}
                    </span>
                  )}
                </div>
              </div>
