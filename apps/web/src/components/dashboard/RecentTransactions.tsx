    ...
    >
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <Link
            href="/transactions"
            className="text-sm text-emerald-600 dark:text-emerald-400 hover:text-emerald-500 dark:hover:text-emerald-300 flex items-center transition-colors duration-200"
          >
            Ver todas
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            {transactions.map((transaction) => (
              <div
                key={transaction._id}
                className="flex items-center justify-between py-3 border-b border-gray-100 dark:border-white/10 last:border-0 transition-colors duration-200"
              >
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm text-gray-500 dark:text-zinc-400 transition-colors duration-200">
                    {format(parseLocalDate(transaction.date), "dd MMM yyyy", {
                      locale: ptBR,
                    })}
                  </p>
                </div>
