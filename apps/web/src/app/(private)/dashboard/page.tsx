...
export default function DashboardPage() {
  ...
  const { data: accountSummary, isLoading: accountLoading } =
    useAccountSummary(isAuthenticated);
  const { data: transactionStats, isLoading: transactionsLoading } =
    useTransactionStats(period, isAuthenticated);
  const { data: goalStats, isLoading: goalsLoading } =
    useGoalStats(isAuthenticated);

  const isDataLoading =
    authLoading || accountLoading || transactionsLoading || goalsLoading;
  ...
  return (
    <div className="space-y-8 p-6 min-h-screen bg-white dark:bg-[#25343b] text-gray-800 dark:text-white transition-colors duration-200">
      ...
      {/* Transações Recentes */}
      <RecentTransactions
        transactions={transactionStats?.chartData
          ?.slice(0, 3)
          .map((_item, index) => ({
            ...
            description:
              index === 0
                ...
                : index === 1
                  ? "Salário"
                  : "Combustível",
            amount: index === 0 ? 425.9 : index === 1 ? 8500.0 : 250.0,
