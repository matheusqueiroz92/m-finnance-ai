...
export default function CreditCardsPage() {
  ...
  } = useCreditCardsList(isAuthenticated);
  const { data: summary, isLoading: summaryLoading } =
    useCreditCardSummary(isAuthenticated);
  const { data: transactions, isLoading: transactionsLoading } =
    useCreditCardTransactions(isAuthenticated);
  const deleteCardMutation = useDeleteCreditCard();

  const isLoading =
    authLoading || cardsLoading || summaryLoading || transactionsLoading;
  ...
  return (
    <div className="space-y-8 p-6 bg-white dark:bg-[#25343b] min-h-screen transition-colors duration-200">
      ...
      {/* Resumo dos Cartões */}
      <CreditCardSummary summary={summary} isLoading={isLoading} />
