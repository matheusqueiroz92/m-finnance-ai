...
export default function GoalsPage() {
  ...
  } = useGoalsList(isAuthenticated);
  const { data: goalStats, isLoading: statsLoading } =
  ...
  const getNextGoal = () => {
    if (!goalStats?.nextGoals || goalStats.nextGoals.length === 0)
      return undefined;
  ...
  return (
    <div className="space-y-8 p-6 bg-white dark:bg-[#25343b] min-h-screen transition-colors duration-200">
      ...
      {/* Estatísticas */}
      <GoalStatCards stats={goalStats} isLoading={isLoading} />
      ...
      {goalStats?.nextGoals && goalStats.nextGoals.length > 0 && (
        <div>
          ...
          <NextGoalCard goal={getNextGoal()} isLoading={isLoading} />
        </div>
      )}
