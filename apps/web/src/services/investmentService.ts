...
export const createInvestment = async (
  data: InvestmentCreateData
): Promise<Investment> => {
  const response = await api.post<ApiResponse<Investment>>(
    API_ROUTES.INVESTMENTS.BASE,
    data
  );
...
export const getInvestmentById = async (id: string): Promise<Investment> => {
  const response = await api.get<ApiResponse<Investment>>(
    API_ROUTES.INVESTMENTS.DETAIL(id)
  );
  return response.data.data;
};
...
export const getInvestmentSummary = async (): Promise<InvestmentSummary> => {
  const response = await api.get<ApiResponse<InvestmentSummary>>(
    API_ROUTES.INVESTMENTS.SUMMARY
  );
  return response.data.data;
};
