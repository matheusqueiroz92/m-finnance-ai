...
export const getGoalStats = async (): Promise<GoalStats> => {
  const response = await api.get<ApiResponse<GoalStats>>(
    API_ROUTES.GOALS.STATS
  );
  return response.data.data;
};
