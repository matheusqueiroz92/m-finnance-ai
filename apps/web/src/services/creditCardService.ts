...
export const createCreditCard = async (
  data: CreditCardCreateData
): Promise<CreditCard> => {
  const response = await api.post<ApiResponse<CreditCard>>(
    API_ROUTES.CREDIT_CARDS.BASE,
    data
  );
...
export const getCreditCards = async (): Promise<CreditCard[]> => {
  const response = await api.get<ApiResponse<CreditCard[]>>(
    API_ROUTES.CREDIT_CARDS.BASE
  );
  return response.data.data;
};
...
export const getCreditCardById = async (id: string): Promise<CreditCard> => {
  const response = await api.get<ApiResponse<CreditCard>>(
    API_ROUTES.CREDIT_CARDS.DETAIL(id)
  );
  return response.data.data;
};
...
export const updateCreditCard = async (
  id: string,
  data: CreditCardUpdateData
): Promise<CreditCard> => {
  const response = await api.put<ApiResponse<CreditCard>>(
    API_ROUTES.CREDIT_CARDS.DETAIL(id),
    data
  );
  return response.data.data;
...
export const getCreditCardSummary = async (): Promise<CreditCardSummary> => {
  const response = await api.get<ApiResponse<CreditCardSummary>>(
    API_ROUTES.CREDIT_CARDS.SUMMARY
  );
  return response.data.data;
};

export const getCreditCardBillings = async (
  creditCardId?: string
...
export const validateSecurityCode = async (
  id: string,
  securityCode: string
): Promise<boolean> => {
